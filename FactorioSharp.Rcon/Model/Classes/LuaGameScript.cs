...
public abstract class LuaGameScript : LuaObject
{
  ...
  /// <summary>
  ///     This property is only populated inside
  ///     <see cref="LuaCommandProcessor)HandlersAndWhenWriting[LuaConsoleCommands](Https://Wiki.Factorio.Com/Console#ScriptingAndCheatCommands" />. Returns the player that is typing
  ///     the command, `nil` in all other instances.
  ...
  /// <summary>
  ///     Get a table of all the players that currently exist. This sparse table allows you to find players by indexing it with either their `name` or `index`. Iterating this table with
  ///     `pairs()` will only iterate the array part of the table. Iterating with `ipairs()` will not work at all.
  ...
  [FactorioRconAttribute("players")]
    public LuaCustomTable<Union973672886, LuaPlayer> Players { get; private set; }
  ...
  ///     This is however not recommended as different difficulties can have differing technology and recipe trees, which can cause problems for players.
  /// </summary>
  ...
  /// <summary>
  ///     Get a table of all the forces that currently exist. This sparse table allows you to find forces by indexing it with either their `name` or `index`. Iterating this table with
  ///     `pairs()` will only iterate the hash part of the table. Iterating with `ipairs()` will not work at all.
  /// </summary>
  [FactorioRconAttribute("forces")]
    public LuaCustomTable<Union973672886, LuaForce> Forces { get; private set; }
  ...
  /// <summary>
  ///     The number of ticks since this game was created using either "new game" or "new game from scenario". Notably, this number progresses even when the game is
  ///     <see cref="LuaGameScript.TickPaused" />.
  ///     This differs from <see cref="LuaGameScript.Tick" /> in that creating a game from a scenario always starts with this value at `0`, even if the scenario has its own level data
  ///     where the `tick` has progressed past `0`.
  /// </summary>
  ...
  /// <summary>
  ///     The number of ticks to be run while the tick is paused. When <see cref="LuaGameScript.TickPaused" /> is true, ticks_to_run behaves the following way: While this is > 0, the
  ///     entity update is running normally and this value is decremented every tick. When this reaches 0, the game will pause again.
  /// </summary>
  ...
  /// <summary>
  ///     Get a table of all the surfaces that currently exist. This sparse table allows you to find surfaces by indexing it with either their `name` or `index`. Iterating this table
  ///     with `pairs()` will only iterate the hash part of the table. Iterating with `ipairs()` will not work at all.
  /// </summary>
  [FactorioRconAttribute("surfaces")]
    public LuaCustomTable<Union973672886, LuaSurface> Surfaces { get; private set; }
  ...
  [FactorioRconAttribute("connected_players")]
    ...
    public abstract void SetGameState(
        bool? gameFinished = null,
        bool? playerWon = null,
        string? nextLevel = null,
        bool? canContinue = null,
        ForceIdentification? victoriousForce = null
    );
    ...
    public abstract void ShowMessageDialog(
        LocalisedString text,
        string? image = null,
        GuiArrowSpecification? pointTo = null,
        string? style = null,
        string? wrapperFrameStyle = null
    );
    ...
    public abstract void RegenerateEntity(Union788293329 entities);

    /// <summary>
    ///     Take a screenshot of the game and save it to the `script-output` folder, located in the game's [user data directory](https://wiki.factorio.com/User_data_directory). The name
    ///     of the image file can be specified via the `path` parameter.
    /// </summary>
    /// <remarks>
    ...
    public abstract void TakeScreenshot(
        ...
        SurfaceIdentification? surface = null,
        MapPosition? position = null,
        TilePosition? resolution = null,
        double? zoom = null,
        string? path = null,
        bool? showGui = null,
        bool? showEntityInfo = null,
        bool? showCursorBuildingPreview = null,
        bool? antiAlias = null,
        int? quality = null,
        bool? allowInReplay = null,
        double? daytime = null,
        uint? waterTick = null,
        bool? forceRender = null
    );
    ...
    /// <summary>
    ///     Take a screenshot of the technology screen and save it to the `script-output` folder, located in the game's [user data
    ///     directory](https://wiki.factorio.com/User_data_directory). The name of the image file can be specified via the `path` parameter.
    /// </summary>
    ...
    public abstract void TakeTechnologyScreenshot(
        ForceIdentification? force = null,
        string? path = null,
        PlayerIdentification? byPlayer = null,
        TechnologyIdentification? selectedTechnology = null,
        bool? skipDisabled = null,
        int? quality = null
    );
    ...
    /// <summary>
    ///     Write a file to the `script-output` folder, located in the game's [user data directory](https://wiki.factorio.com/User_data_directory). The name and file extension of the file
    ...
    /// <summary>
    ///     Force a CRC check. Tells all peers to calculate their current CRC, which are then compared to each other. If a mismatch is detected, the game desyncs and some peers are forced
    ///     to reconnect.
    /// </summary>
    ...
    /// <summary>
    ///     Marks two forces to be merged together. All players and entities in the source force will be reassigned to the target force. The source force will then be destroyed.
    ///     Importantly, this does not merge technologies or bonuses, which are instead retained from the target force.
    /// </summary>
    /// <remarks>
    ...
    public abstract List<LuaEntity> GetTrainStops(Union788293329? name = null, SurfaceIdentification? surface = null, ForceIdentification? force = null);
    ...
    public abstract LuaPlayer? GetPlayer(Union973672886 player);
    ...
    public abstract LuaSurface? GetSurface(Union973672886 surface);
    ...
    public abstract Union625632831 RequestTrainPath(
        List<Union45416710> goals,
        bool? inChainSignalSection = null,
        LuaTrain? train = null,
        TrainPathRequestType? type = null,
        bool? returnPath = null,
        RailEnd? fromFront = null,
        bool? allowPathWithinSegmentFront = null,
        RailEnd? fromBack = null,
        bool? allowPathWithinSegmentBack = null,
        Literals1861556821? searchDirection = null,
        uint? stepsLimit = null
    );
}
