...
/// <summary>
///     A "domain" of the world. Surfaces can only be created and deleted through the API. Surfaces are uniquely identified by their name. Every game contains at least the surface
///     "nauvis".
/// </summary>
...
public abstract class LuaSurface : LuaObject
{
  ...
  /// <summary>
  ///     The generation settings for this surface. These can be modified after surface generation, but note that this will not retroactively update the surface. To manually regenerate
  ///     it, <see cref="LuaSurface.DeleteChunk" /> can be used.
  /// </summary>
  ...
  [FactorioRconMethod("can_place_entity")]
    public abstract bool CanPlaceEntity(
        string name,
        MapPosition position,
        DirectionEnum? direction = null,
        ForceIdentification? force = null,
        BuildCheckTypeEnum? buildCheckType = null,
        bool? forced = null,
  ...
  ///     If no filters (`name`, `type`, `force`, etc.) are given, this returns all entities in the search area. If multiple filters are specified, only entities matching all given
  ...
  [FactorioRconMethod("find_entities_filtered")]
    public abstract List<LuaEntity> FindEntitiesFiltered(
        ...
        double? radius = null,
        Union788293329? name = null,
        Union788293329? type = null,
        Union788293329? ghostName = null,
        Union788293329? ghostType = null,
        Union1558479331? direction = null,
        Union1671383842? collisionMask = null,
        Union1241242689? force = null,
        bool? toBeDeconstructed = null,
        bool? toBeUpgraded = null,
        uint? limit = null,
        bool? isMilitaryTarget = null,
        LuaItemPrototype? hasItemInside = null,
        bool? invert = null
    );
  ...
  ///     If no `area` or `position` and `radius` is given, the entire surface is searched. If `position` and `radius` are given, only tiles within the radius of the position are
  ///     included.
  /// </summary>
  /// <param name="area">Lua name: area</param>
  /// <param name="position">Lua name: position</param>
  ...
  [FactorioRconMethod("find_tiles_filtered")]
    public abstract List<LuaTile> FindTilesFiltered(
        ...
        Union1241242689? force = null,
        uint? limit = null,
        bool? hasHiddenTile = null,
        bool? hasTileGhost = null,
        bool? toBeDeconstructed = null,
        Union1671383842? collisionMask = null,
        bool? invert = null
    );
  ...
  /// <summary>
  ///     Count entities of given type or name in a given area. Works just like <see cref="LuaSurface.FindEntitiesFiltered" />, except this only returns the count. As it doesn't
  ///     construct all the wrapper objects, this is more efficient if one is only interested in the number of entities.
  ///     If no `area` or `position` are given, the entire surface is searched. If `position` is given, this returns the entities colliding with that position (i.e the given position is
  ///     within the entity's collision box). If `position` and `radius` are given, this returns entities in the radius of the position. If `area` is specified, this returns entities
  ...
  [FactorioRconMethod("count_entities_filtered")]
    public abstract uint CountEntitiesFiltered(
        BoundingBox? area = null,
        MapPosition? position = null,
        double? radius = null,
        Union788293329? name = null,
  ...
  /// <summary>
  ///     Count tiles of a given name in a given area. Works just like <see cref="LuaSurface.FindTilesFiltered" />, except this only returns the count. As it doesn't construct all the
  ///     wrapper objects, this is more efficient if one is only interested in the number of tiles.
  ///     If no `area` or `position` and `radius` is given, the entire surface is searched. If `position` and `radius` are given, only tiles within the radius of the position are
  ///     included.
  /// </summary>
  /// <param name="area">Lua name: area</param>
  ...
  [FactorioRconMethod("count_tiles_filtered")]
    public abstract uint CountTilesFiltered(
        BoundingBox? area = null,
        MapPosition? position = null,
        double? radius = null,
        Union788293329? name = null,
  ...
  /// </summary>
  /// <remarks>
  ///     Special care needs to be taken when using a radius of `0`. The game will not stop searching until it finds a suitable position, so it is important to make sure such a position
  ///     exists. One particular case where it would not be able to find a solution is running it before any chunks have been generated.
  /// </remarks>
  /// <param name="name">Lua name: name</param>
  ...
  [FactorioRconMethod("spill_item_stack")]
    public abstract List<LuaEntity> SpillItemStack(
        MapPosition position,
        ItemStackIdentification items,
        bool? enableLooted = null,
        ForceIdentification? force = null,
        bool? allowBelts = null
    );
  ...
  [FactorioRconMethod("create_entity")]
    public abstract LuaEntity? CreateEntity(
        string name,
        MapPosition position,
        DirectionEnum? direction = null,
        ForceIdentification? force = null,
        Union955315863? target = null,
        Union955315863? source = null,
        bool? fastReplace = null,
        PlayerIdentification? player = null,
        LuaEntity? character = null,
        bool? spill = null,
        bool? raiseBuilt = null,
        bool? createBuildEffectSmoke = null,
        bool? spawnDecorations = null,
        bool? moveStuckPlayers = null,
        LuaItemStack? item = null
    );
  ...
  /// </summary>
  /// <remarks>
  ///     It is recommended to call this method once for all the tiles you want to change rather than calling it individually for every tile. As the tile correction is used after every
  ///     step, calling it one by one could cause the tile correction logic to redo some of the changes. Also, many small API calls are generally more performance intensive than one big
  ///     one.
  /// </remarks>
  ...
  [FactorioRconMethod("set_tiles")]
    public abstract void SetTiles(
        List<Tile> tiles,
        bool? correctTiles = null,
        Union401330273? removeCollidingEntities = null,
        bool? removeCollidingDecoratives = null,
  ...
  /// <summary>
  ///     Request that the game's map generator generate chunks at the given position for the given radius on this surface. If the radius is `0`, then only the chunk at the given
  ...
  [FactorioRconMethod("cancel_deconstruct_area")]
    public abstract void CancelDeconstructArea(
        BoundingBox area,
        ForceIdentification force,
        PlayerIdentification? player = null,
        bool? skipFogOfWar = null,
        LuaItemStack? item = null
    );
  ...
  /// <summary>
  ///     Set the hidden tile for the specified position. While during normal gameplay only <see cref="LuaTilePrototype.MineableProperties" /> tiles can become hidden, this method
  ///     allows any kind of tile to be set as the hidden one.
  /// </summary>
  /// <param name="position">Lua name: position</param>
  /// <param name="tile">Lua name: tile</param>
  [FactorioRconMethod("set_hidden_tile")]
    public abstract void SetHiddenTile(TilePosition position, Union1651295739 tile);
  ...
  [FactorioRconMethod("get_connected_tiles")]
    ...
    public abstract void RegenerateEntity(Union788293329? entities = null, List<ChunkPosition>? chunks = null);
    ...
    public abstract void RegenerateDecorative(Union788293329? decoratives = null, List<ChunkPosition>? chunks = null);
    ...
    public abstract void DestroyDecoratives(
        BoundingBox? area = null,
        TilePosition? position = null,
        Union1980377764? name = null,
        Union1671383842? collisionMask = null,
        string? fromLayer = null,
        string? toLayer = null,
        bool? excludeSoft = null,
        uint? limit = null,
        bool? invert = null
    );
    ...
    ///     If no filters are given, returns all decoratives in the search area. If multiple filters are specified, returns only decoratives matching every given filter. If no area and no
    ///     position are given, the entire surface is searched.
    /// </summary>
    /// <param name="area">Lua name: area</param>
    /// <param name="position">Lua name: position</param>
    /// <param name="name">Lua name: name</param>
    ...
    public abstract List<DecorativeResult> FindDecorativesFiltered(
        BoundingBox? area = null,
        TilePosition? position = null,
        Union1980377764? name = null,
        Union1671383842? collisionMask = null,
    ...
    public abstract void CloneArea(
        BoundingBox sourceArea,
        BoundingBox destinationArea,
        SurfaceIdentification? destinationSurface = null,
        ForceIdentification? destinationForce = null,
        bool? cloneTiles = null,
        bool? cloneEntities = null,
        bool? cloneDecoratives = null,
        bool? clearDestinationEntities = null,
        bool? clearDestinationDecoratives = null,
        bool? expandMap = null,
        bool? createBuildEffectSmoke = null
    );
    ...
    public abstract void CloneBrush(
        ...
        TilePosition destinationOffset,
        List<TilePosition> sourcePositions,
        SurfaceIdentification? destinationSurface = null,
        Union1333519790? destinationForce = null,
        bool? cloneTiles = null,
        bool? cloneEntities = null,
        bool? cloneDecoratives = null,
        bool? clearDestinationEntities = null,
    ...
    public abstract void CloneEntities(
        List<LuaEntity> entities,
    ...
    public abstract uint RequestPath(
        BoundingBox boundingBox,
        Union520878677 collisionMask,
        MapPosition start,
        MapPosition goal,
        ForceIdentification force,
        double? radius = null,
        PathfinderFlags? pathfindFlags = null,
        bool? canOpenGates = null,
        int? pathResolutionModifier = null,
        LuaEntity? entityToIgnore = null
    );
    ...
    public abstract ScriptArea? GetScriptArea(Union35422357? key = null);
    ...
    public abstract ScriptPosition? GetScriptPosition(Union35422357? key = null);
    ...
    public abstract List<LuaEntity> GetTrainStops(Union788293329? name = null, ForceIdentification? force = null);
