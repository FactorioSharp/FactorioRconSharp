...
public abstract class LuaBootstrap : LuaObject
{
  ...
  /// <summary>
  ///     Register a function to be run on mod initialization. This is only called when a new save game is created or when a save file is loaded that previously didn't contain the mod.
  ///     During it, the mod gets the chance to set up initial values that it will use for its lifetime. It has full access to <see cref="Global" /> table and can change anything about
  ///     them that it deems appropriate. No other events will be raised for the mod until it has finished this step.
  /// </summary>
  /// <remarks>
  ...
  /// <summary>
  ///     Register a function to be run on save load. This is only called for mods that have been part of the save previously, or for players connecting to a running multiplayer
  ///     session.
  ///     It gives the mod the opportunity to rectify potential differences in local state introduced by the save/load cycle. Doing anything other than the following three will lead to
  ///     desyncs, breaking multiplayer and replay functionality. Access to <see cref="Global" /> table can be accessed and is safe to read from, but not write to, as doing so will lead
  ///     to an error.
  ...
  /// <summary>
  ///     Register a function to be run when mod configuration changes. This is called when the game version or any mod version changed, when any mod was added or removed, when a
  ///     startup setting has changed, when any prototypes have been added or removed, or when a migration was applied. It allows the mod to make any changes it deems appropriate to
  ///     both the data structures in its <see cref="LuaGameScript" />.
  /// </summary>
  /// <remarks>
  ///     For more context, refer to the <see cref="DataLifecycle" /> page.
  /// </remarks>
  ...
  /// <summary>
  ///     Register a handler to run on the specified event(s). Each mod can only register once for every event, as any additional registration will overwrite the previous one. This
  ///     holds true even if different filters are used for subsequent registrations.
  /// </summary>
  ...
  /// <summary>
  ///     Registers an entity so that after it's destroyed, <see cref="OnEntityDestroyed" /> will receive the event when it is destroyed. Registering the same entity multiple times will
  ///     still only fire the destruction event once, and will return the same registration number.
  /// </summary>
  /// <remarks>
  ...
  /// <summary>
  ///     Register a metatable to have linkage recorded and restored when saving/loading. The metatable itself will not be saved. Instead, only the linkage to a registered metatable is
  ///     saved, and the metatable registered under that name will be used when loading the table.
  /// </summary>
  /// <remarks>
  ...
  /// <summary>
  ///     Sets the filters for the given event. The filters are only retained when set after the actual event registration, because registering for an event with different or no filters
  ///     will overwrite previously set ones.
  /// </summary>
  /// <param name="@event">Lua name: event</param>
  /// <param name="filters">Lua name: filters</param>
