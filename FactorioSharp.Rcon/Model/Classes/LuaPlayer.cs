...
/// <summary>
///     A player in the game. Pay attention that a player may or may not have a character, which is the <see cref="LuaEntity" /> of the little guy running around the world doing
///     things.
/// </summary>
...
public abstract class LuaPlayer : LuaObject
{
  ...
  /// <summary>
  ///     When in a cutscene; the character this player would be using once the cutscene is over, if any. Returns `nil` when the player is disconnected (see
  ///     <see cref="LuaPlayer.Connected)" />.
  /// </summary>
  ...
  [FactorioRconAttribute("index")]
    ...
    /// <summary>
    ///     The current per-player settings for the this player, indexed by prototype name. Returns the same structure as <see cref="LuaSettings.GetPlayerSettings" />. This table becomes
    ///     invalid if its associated player does.
    ///     Even though this attribute is marked as read-only, individual settings can be changed by overwriting their <see cref="ModSetting" /> table. Mods can only change their own
    ...
    /// <summary>
    ///     The original location of the item in the cursor, marked with a hand. `nil` if the cursor stack is empty. When writing, the specified inventory slot must be empty and the
    ///     cursor stack must not be empty.
    /// </summary>
    [FactorioRconAttribute("hand_location")]
    public ItemStackLocation HandLocation { get; set; }

    /// <summary>
    ///     Returns true if the current item stack in cursor will be destroyed after clearing the cursor. Manually putting it into inventory still preserves the item. If the cursor stack
    ///     is not one of the supported types (blueprint, blueprint-book, deconstruction-planner, upgrade-planner), write operation will be silently ignored.
    /// </summary>
    ...
    public abstract void SetController(
        ControllersEnum type,
        LuaEntity? character = null,
        CutsceneWaypoint? waypoints = null,
        MapPosition? startPosition = null,
        double? startZoom = null,
        uint? finalTransitionTime = null,
        double? chartModeCutoff = null
    );
    ...
    /// <summary>
    ///     Unlock the achievements of the given player. This has any effect only when this is the local player, the achievement isn't unlocked so far and the achievement is of the type
    ///     "achievement".
    /// </summary>
    ...
    public abstract void RemoveAlert(
        LuaEntity? entity = null,
        Union844977329? prototype = null,
        MapPosition? position = null,
        AlertTypeEnum? type = null,
        SurfaceIdentification? surface = null,
        SignalID? icon = null,
        LocalisedString? message = null
    );
    ...
    public abstract Dictionary<uint, Dictionary<AlertTypeEnum, List<Alert>>> GetAlerts(
        LuaEntity? entity = null,
        LuaEntityPrototype? prototype = null,
        MapPosition? position = null,
        AlertTypeEnum? type = null,
        SurfaceIdentification? surface = null
    );
    ...
    public abstract bool PipetteEntity(Union1265061987 entity);
    ...
    /// <summary>
    ///     Spawn flying text that is only visible to this player. Either `position` or `create_at_cursor` are required. When `create_at_cursor` is `true`, all parameters other than
    ///     `text` are ignored.
    /// </summary>
    /// <remarks>
    ...
    public abstract void CreateLocalFlyingText(
        LocalisedString text,
        MapPosition? position = null,
        bool? createAtCursor = null,
        Color? color = null,
    ...
    public abstract void SetQuickBarSlot(uint index, Union1054026747 filter);
    ...
    /// <summary>
    ///     Queues a request to open the map at the specified position. If the map is already opened, the request will simply set the position, scale, and entity to follow. Render mode
    ///     change requests are processed before rendering of the next frame.
    /// </summary>
    /// <param name="position">Lua name: position</param>
    ...
    /// <summary>
    ///     Queues a request to zoom to world at the specified position. If the player is already zooming to world, the request will simply set the position, scale, and entity to follow.
    ...
    /// <summary>
    ///     Requests translation for the given set of localised strings. If the request is successful, a <see cref="OnStringTranslated" /> event will be fired for each string with the
    ///     results.
    /// </summary>
    /// <remarks>
    ///     Does nothing if this player is not connected (see <see cref="LuaPlayer.Connected)" />.
    ...
    public abstract void SetInfinityInventoryFilter(uint index, Union532119748 filter);
