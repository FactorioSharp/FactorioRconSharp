...
public abstract class LuaEntity : LuaObject
{
    ...
    /// <summary>
    ///     The current speed if this is a car, rolling stock, projectile or spidertron, or the maximum speed if this is a unit. The speed is in tiles per tick. `nil` if this is not a
    ///     car, rolling stock, unit, projectile or spidertron.
    ...
    public Union1459943261 GhostPrototype { get; private set; }
    ...
    /// <summary>
    ///     The entity this entity is putting its items to. If there are multiple possible entities at the drop-off point, writing to this attribute allows a mod to choose which one to
    ///     drop off items to. The entity needs to collide with the tile box under the drop-off position. `nil` if there is no entity to put items to, or if this is not an entity that
    ...
    /// <summary>
    ///     The entity this inserter will attempt to pick up items from. If there are multiple possible entities at the pick-up point, writing to this attribute allows a mod to choose
    ///     which one to pick up items from. The entity needs to collide with the tile box under the pick-up position. `nil` if there is no entity to pull items from.
    /// </summary>
    ...
    /// <summary>
    ///     A list of neighbours for certain types of entities. Applies to electric poles, power switches, underground belts, walls, gates, reactors, cliffs, and pipe-connectable
    ///     entities.
    ...
    public Union1339664247 Neighbours { get; private set; }

    /// <summary>
    ///     The belt connectable neighbours of this belt connectable entity. Only entities that input to or are outputs of this entity. Does not contain the other end of an underground
    ///     belt, see <see cref="LuaEntity.Neighbours" /> for that.
    /// </summary>
    [FactorioRconAttribute("belt_neighbours")]
    public Table1930046673 BeltNeighbours { get; private set; }
    ...
    /// <summary>
    ///     The backer name assigned to this entity. Entities that support backer names are labs, locomotives, radars, roboports, and train stops. `nil` if this entity doesn't support
    ///     backer names.
    /// </summary>
    ...
    public Literals745161356 BeltToGroundType { get; private set; }

    /// <summary>
    ///     Whether this loader gets items from or puts item into a container.
    /// </summary>
    [FactorioRconAttribute("loader_type")]
    public Literals1975889157 LoaderType { get; set; }
    ...
    /// <summary>
    ///     The last player that changed any setting on this entity. This includes building the entity, changing its color, or configuring its circuit network. `nil` if the last user is
    ///     not part of the save anymore.
    ...
    public Union1098069758 LastUser { get; set; }
    ...
    ///     Only entities inheriting from [EntityWithOwnerPrototype](prototype:EntityWithOwnerPrototype), as well as [ItemRequestProxyPrototype](prototype:ItemRequestProxyPrototype) and
    ...
    /// <summary>
    ///     The <see cref="LuaEntity.UnitNumber)OfTheEntityContainedInThisGhost.ItIsTheSameAsTheUnitNumberOfThe[EntityWithOwnerPrototype](Prototype:EntityWithOwnerPrototype" /> that was
    ///     destroyed to create this ghost. If it was created by other means, or if the inner entity does not support unit numbers, this property is `nil`.
    /// </summary>
    ...
    /// <summary>
    ///     The bonus mining progress for this mining drill. Read yields a number in range [0, mining_target.prototype.mineable_properties.mining_time]. `nil` if this isn't a mining
    ///     drill.
    /// </summary>
    ...
    public Table686901828 CircuitConnectedEntities { get; private set; }
    ...
    public Table1695062348 StickerVehicleModifiers { get; private set; }
    ...
    ///     The player will be automatically disassociated when a controller is set on the character. Also, all characters associated to a player will be logged off when the player logs
    ///     off in multiplayer.
    ///     Reading this property will return a <see cref="PlayerIdentification" /> can be used when writing.
    /// </summary>
    [FactorioRconAttribute("associated_player")]
    public Union1098069758 AssociatedPlayer { get; set; }
    ...
    /// <summary>
    ///     The player that this `simple-entity-with-owner`, `simple-entity-with-force`, `flying-text`, or `highlight-box` is visible to. `nil` when this entity is rendered for all
    ///     players.
    ///     Reading this property will return a <see cref="PlayerIdentification" /> can be used when writing.
    /// </summary>
    [FactorioRconAttribute("render_player")]
    public Union1098069758 RenderPlayer { get; set; }

    /// <summary>
    ///     The forces that this `simple-entity-with-owner`, `simple-entity-with-force`, or `flying-text` is visible to. `nil` or an empty array when this entity is rendered for all
    ///     forces.
    /// </summary>
    ...
    /// <summary>
    ///     If true, corpse won't be destroyed when entities are placed over it. If false, whether corpse will be removed or not depends on value of
    ///     CorpsePrototype::remove_on_entity_placement.
    /// </summary>
    ...
    /// <summary>
    ///     Amount of trains related to this particular train stop. Includes train stopped at this train stop (until it finds a path to next target) and trains having this train stop as
    ///     goal or waypoint.
    /// </summary>
    ...
    public Literals1615578818 LinkedBeltType { get; set; }
    ...
    ///     dimension). Uses the current direction of the entity.
    /// </summary>
    ...
    public Literals1101482515 BeltShape { get; private set; }
    ...
    public abstract bool ConnectNeighbour(Union851287548 target);
    ...
    public abstract void DisconnectNeighbour(Union1812046695? target = null);
    ...
    /// <summary>
    ///     Returns whether a craft is currently in process. It does not indicate whether progress is currently being made, but whether a crafting process has been started in this
    ///     machine.
    /// </summary>
    ...
    public abstract (LuaEntity?, RailDirectionEnum?, RailConnectionDirectionEnum?) GetConnectedRail(
        RailDirectionEnum railDirection,
        RailConnectionDirectionEnum railConnectionDirection
    );
    ...
    /// <summary>
    ///     Returns all parent signals. Parent signals are always RailChainSignal. Parent signals are those signals that are checking state of this signal to determine their own chain
    ///     state.
    /// </summary>
    ...
    public abstract void SetFilter(uint slotIndex, Union1109066428 item);
    ...
    public abstract void SetInfinityContainerFilter(uint index, Union532119748 filter);
    ...
    public abstract void SetInfinityPipeFilter(Union1023877861 filter);
    ...
    public abstract Union1603896041? GetLogisticPoint(LogisticMemberIndexEnum? index = null);
    ...
    /// <summary>
    ///     Reconnect loader, beacon, cliff and mining drill connections to entities that might have been teleported out or in by the script. The game doesn't do this automatically as we
    ///     don't want to loose performance by checking this in normal games.
    /// </summary>
    ...
    public abstract Dictionary<string, uint> SetRecipe(Union493873923 recipe);
    ...
    public abstract (bool, Dictionary<string, uint>?) Rotate(
        bool? reverse = null,
        PlayerIdentification? byPlayer = null,
        bool? spillItems = null,
        bool? enableLooted = null,
        ForceIdentification? force = null
    );
    ...
    public abstract Union497573081? GetDriver();
    ...
    public abstract void SetDriver(Union458526948? driver = null);
    ...
    public abstract Union497573081? GetPassenger();
    ...
    public abstract void SetPassenger(Union458526948? passenger = null);
    ...
    public abstract void SetBeamSource(Union955315863 source);
    ...
    public abstract void SetBeamTarget(Union955315863 target);
    ...
    /// <summary>
    ///     Creates the same smoke that is created when you place a building by hand. You can play the building sound to go with it by using <see cref="LuaSurface.PlaySound" />, eg:
    ...
    /// <summary>
    ///     Only works if the entity is a speech-bubble, with an "effect" defined in its wrapper_flow_style. Starts animating the opacity of the speech bubble towards zero, and destroys
    ///     the entity when it hits zero.
    /// </summary>
    ...
    /// <summary>
    ///     Is this entity or tile ghost or item request proxy registered for construction? If false, it means a construction robot has been dispatched to build the entity, or it is not
    ///     an entity that can be constructed.
    /// </summary>
    [FactorioRconMethod("is_registered_for_construction")]
    public abstract bool IsRegisteredForConstruction();

    /// <summary>
    ///     Is this entity registered for deconstruction with this force? If false, it means a construction robot has been dispatched to deconstruct it, or it is not marked for
    ///     deconstruction. The complexity is effectively O(1) - it depends on the number of objects targeting this entity which should be small enough.
    /// </summary>
    /// <param name="force">Lua name: force</param>
    ...
    /// <summary>
    ///     Is this entity registered for upgrade? If false, it means a construction robot has been dispatched to upgrade it, or it is not marked for upgrade. This is worst-case O(N)
    ///     complexity where N is the current number of things in the upgrade queue.
    /// </summary>
    [FactorioRconMethod("is_registered_for_upgrade")]
    public abstract bool IsRegisteredForUpgrade();

    /// <summary>
    ///     Is this entity registered for repair? If false, it means a construction robot has been dispatched to upgrade it, or it is not damaged. This is worst-case O(N) complexity where
    ///     N is the current number of things in the repair queue.
    /// </summary>
    ...
    ///     Neighbours have to be of different type. If given linked belt is connected to something else it will be disconnected first. If provided neighbour is connected to something
    ///     else it will also be disconnected first. Automatically updates neighbour to be connected back to this one.
    /// </summary>
    /// <remarks>
