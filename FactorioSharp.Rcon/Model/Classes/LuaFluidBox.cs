...
/// <summary>
///     An array of fluid boxes of an entity. Entities may contain more than one fluid box, and some can change the number of fluid boxes -- for instance, an assembling machine will
///     change its number of fluid boxes depending on its active recipe. See <see cref="Fluid" />.
///     Do note that reading from a <see cref="LuaFluidBox" /> creates a new table and writing will copy the given fields from the table into the engine's own fluid box structure.
///     Therefore, the correct way to update a fluidbox of an entity is to read it first, modify the table, then write the modified table back. Directly accessing the returned table's
...
public abstract class LuaFluidBox : LuaObject
{
  ...
  /// <summary>
  ///     Access, set or clear a fluid box. The index must always be in bounds (see <see cref="LuaFluidBox.LengthOperator)" />. New fluidboxes may not be added or removed using this
  ///     operator.
  ...
  /// <summary>
  ///     The prototype of this fluidbox index. If this is used on a fluidbox of a crafting machine which due to recipe was created by merging multiple prototypes, a table of prototypes
  ///     that were merged will be returned instead
  /// </summary>
  /// <param name="index">Lua name: index</param>
  [FactorioRconMethod("get_prototype")]
    public abstract Union216791007 GetPrototype(uint index);
  ...
  [FactorioRconMethod("set_filter")]
    public abstract bool SetFilter(uint index, Union1751895091 filter);
