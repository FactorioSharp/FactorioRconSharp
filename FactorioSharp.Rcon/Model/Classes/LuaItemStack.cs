...
public abstract class LuaItemStack : LuaObject
{
  ...
  [FactorioRconAttribute("prioritize_insertion_mode")]
    public Literals2012425039 PrioritizeInsertionMode { get; set; }
  ...
  [FactorioRconAttribute("default_icons")]
    ...
    /// <summary>
    ///     The entity filters for this deconstruction item. The attribute is a sparse array with the keys representing the index of the filter. All strings in this array must be entity
    ///     prototype names that don't have the `"not-deconstructable"` flag set and are either a `cliff` or marked as `minable`.
    /// </summary>
    [FactorioRconAttribute("entity_filters")]
    public List<string> EntityFilters { get; set; }

    /// <summary>
    ///     The tile filters for this deconstruction item. The attribute is a sparse array with the keys representing the index of the filter. All strings in this array must be tile
    ///     prototype names.
    /// </summary>
    ...
    /// <remarks>
    ///     Built entities can be come invalid between the building of the blueprint and the function returning if by_player or raise_built is used and one of those events invalidates the
    ///     entity.
    /// </remarks>
    ...
    public abstract List<LuaEntity> BuildBlueprint(
        SurfaceIdentification surface,
        ForceIdentification force,
        MapPosition position,
        bool? forceBuild = null,
    ...
    public abstract void DeconstructArea(
        SurfaceIdentification surface,
        ForceIdentification force,
        BoundingBox area,
        bool? skipFogOfWar = null,
    ...
    public abstract Dictionary<uint, LuaEntity> CreateBlueprint(
        SurfaceIdentification surface,
        ForceIdentification force,
        BoundingBox area,
        bool? alwaysIncludeTiles = null,
        bool? includeEntities = null,
        bool? includeModules = null,
        bool? includeStationNames = null,
        bool? includeTrains = null,
        bool? includeFuel = null
    );
    ...
    public abstract bool SetEntityFilter(uint index, Union1641951594 filter);
    ...
    public abstract bool SetTileFilter(uint index, Union1444717342 filter);
    ...
    public abstract UpgradeFilter GetMapper(uint index, Literals1744116963 type);
    ...
    public abstract void SetMapper(uint index, Literals137692570 type, Union2036427321 filter);
