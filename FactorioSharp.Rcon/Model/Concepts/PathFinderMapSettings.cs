...
public class PathFinderMapSettings
{
  ...
  /// <summary>
  ///     When looking at which node to check next, their heuristic value is multiplied by this ratio. The higher it is, the more the search is directed straight at the goal. Defaults
  ///     to `2`.
  /// </summary>
  ...
  /// <summary>
  ///     When looking for a path from cache, make sure it doesn't end too far from the requested end in relative terms. This is typically more lenient than the start ratio since the
  ///     end target could be moving. Defaults to `0.15`.
  /// </summary>
  ...
  /// <summary>
  ///     When assigning a rating to the best path, this multiplier times end distances is considered. This value is typically higher than the start multiplier as this results in better
  ///     end path quality. Defaults to `20`.
  /// </summary>
  ...
  /// <summary>
  ///     A penalty that is applied for another unit that is on the way to the goal. This is mainly relevant for situations where a group of units has arrived at the target they are
  ///     supposed to attack, making units further back circle around to reach the target. Defaults to `30`.
  /// </summary>
  ...
  /// <summary>
  ///     When the `max_clients_to_accept_any_new_request` amount is exhausted, only path finder requests with a short estimate will be accepted until this amount (per tick) is reached.
  ///     Defaults to `100`.
  /// </summary>
  ...
  /// <summary>
  ///     The thresholds of waiting clients after each of which the per-tick work limit will be increased by the corresponding value in `overload_multipliers`. This is to avoid clients
  ///     having to wait too long. Must have the same number of elements as `overload_multipliers`. Defaults to `{0, 100, 500}`.
  /// </summary>
  [FactorioRconAttribute("overload_levels")]
    public List<uint> OverloadLevels { get; set; }

  /// <summary>
  ///     The multipliers to the amount of per-tick work applied after the corresponding thresholds in `overload_levels` have been reached. Must have the same number of elements as
  ///     `overload_multipliers`. Defaults to `{2, 3, 4}`.
  /// </summary>
